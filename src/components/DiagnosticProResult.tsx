...
const urgencyConfig = {
  faible: { label: 'Faible', color: 'bg-green-500', textColor: 'text-green-600', bgColor: 'bg-green-950/30', icon: '🟢' },
  moyen: { label: 'Moyen', color: 'bg-yellow-500', textColor: 'text-yellow-600', bgColor: 'bg-yellow-950/30', icon: '🟡' },
  urgent: { label: 'Urgent', color: 'bg-red-500', textColor: 'text-red-600', bgColor: 'bg-red-950/30', icon: '🔴' }
}
...
const probabilityConfig = {
  'élevée': { label: 'Haute', color: 'bg-red-100 bg-red-900/30 dark:text-red-400' },
  'moyenne': { label: 'Moyenne', color: 'bg-yellow-100 bg-yellow-900/30 dark:text-yellow-400' },
  'faible': { label: 'Faible', color: 'bg-green-100 bg-green-900/30 dark:text-green-400' }
}
...
export default function DiagnosticProResult({ diagnosis }: DiagnosticProResultProps) {
  ...
  return (
    <div className="bg-neutral-900 rounded-xl md:rounded-2xl border border-neutral-800 overflow-hidden shadow-xl">
      {/* Header */}
      ...
      <div className="p-3 md:p-5 space-y-3 md:space-y-5">
        ...
        {diagnosis.problem_identified && (
          <div>
            <h3 className="text-xs md:text-sm font-semibold text-neutral-400 mb-1.5 md:mb-2 flex items-center gap-1.5 md:gap-2">
              <span>🎯</span> Problème identifié
            </h3>
            <p className="text-xs md:text-base text-neutral-200">{diagnosis.problem_identified}</p>
          </div>
        )}
        ...
        {diagnosis.causes_possibles && diagnosis.causes_possibles.length > 0 && (
          <div>
            <h3 className="text-xs md:text-sm font-semibold text-neutral-400 mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2">
            ...
            <div className="space-y-2 md:space-y-3">
              {diagnosis.causes_possibles.map((cause, i) => {
                ...
                return (
                  <div key={i} className="bg-neutral-800 rounded-lg md:rounded-xl p-2.5 md:p-4">
                    <div className="flex items-start justify-between gap-2 md:gap-3 mb-1 md:mb-2">
                      <span className="font-medium text-xs md:text-base text-neutral-200">{cause.cause}</span>
                    ...
                    <p className="text-[10px] md:text-sm text-neutral-400">{cause.explication}</p>
                  </div>
                )
              })}
            </div>
        ...
        {/* Cost Estimate */}
        <div className="bg-neutral-800 rounded-lg md:rounded-xl p-2.5 md:p-4">
          <h3 className="text-xs md:text-sm font-semibold text-neutral-400 mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2">
            <span>💰</span> Estimation financière
          </h3>
          <div className="flex items-center justify-between">
            <span className="text-xs md:text-base text-neutral-400">Coût estimé</span>
            <span className="text-lg md:text-2xl font-bold text-white">
              {diagnosis.estimated_cost_min} - {diagnosis.estimated_cost_max} €
            </span>
          </div>
          {diagnosis.time_estimate && (
            <div className="flex items-center justify-between mt-1.5 md:mt-2 pt-1.5 md:pt-2 border-t border-neutral-700">
              <span className="text-[10px] md:text-sm text-neutral-500">Temps estimé</span>
              <span className="text-[10px] md:text-sm font-medium text-neutral-300">{diagnosis.time_estimate}</span>
            </div>
          )}
        </div>
          ...
          <div>
            ...
            <div className="space-y-1.5 md:space-y-2">
              {diagnosis.parts_needed.map((part, i) => (
                <div key={i} className="flex items-center justify-between p-2 md:p-3 bg-neutral-800 rounded-lg">
                  <div className="min-w-0 flex-1">
                    <span className="text-xs md:text-base text-neutral-200 block truncate">{part.name}</span>
                    {part.reference && (
                      <span className="text-[9px] md:text-xs text-neutral-500">Réf: {part.reference}</span>
                    )}
                  </div>
                  <span className="font-medium text-xs md:text-base text-violet-400 ml-2 flex-shrink-0">{part.price_estimate}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {diagnosis.tsb_found && diagnosis.tsb_found.length > 0 && (
          <div className="bg-amber-950/30 rounded-lg md:rounded-xl p-2.5 md:p-4 border border-amber-800">
            <h3 className="text-xs md:text-sm font-semibold text-amber-400 mb-2 md:mb-3 flex items-center gap-1.5 md:gap-2">
            ...
            <div className="space-y-1.5 md:space-y-2">
              {diagnosis.tsb_found.map((tsb, i) => (
                <div key={i} className="text-[10px] md:text-sm">
                  <div className="font-medium text-amber-300">{tsb.reference}</div>
                  <p className="text-amber-400">{tsb.description}</p>
                  {tsb.url && (
            ...
            <ol className="space-y-1.5 md:space-y-2">
              {diagnosis.recommandations.map((rec, i) => (
                <li key={i} className="flex items-start gap-2 md:gap-3 text-neutral-300">
                  <span className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-violet-900/30 text-violet-400 flex items-center justify-center text-[10px] md:text-xs font-semibold flex-shrink-0">
                    {i + 1}
                  </span>
        ...
        {diagnosis.risks_if_ignored && diagnosis.risks_if_ignored.length > 0 && (
          <div className="bg-red-950/30 rounded-lg md:rounded-xl p-2.5 md:p-4 border border-red-800">
            <h3 className="text-xs md:text-sm font-semibold text-red-400 mb-1.5 md:mb-2 flex items-center gap-1.5 md:gap-2">
              <span>⚠️</span> Risques si non traité
            </h3>
            <ul className="space-y-0.5 md:space-y-1">
              {diagnosis.risks_if_ignored.map((risk, i) => (
                <li key={i} className="flex items-start gap-1.5 md:gap-2 text-[10px] md:text-sm text-red-400">
                  <span>•</span>
                  <span>{risk}</span>
                </li>
              ))}
        ...
        {/* DIY Difficulty */}
        {difficulty && (
          <div className="flex items-center justify-between p-2.5 md:p-4 bg-neutral-800 rounded-lg md:rounded-xl">
            <span className="text-[10px] md:text-sm text-neutral-400 flex items-center gap-1.5 md:gap-2">
          ...
          <div>
            ...
            <div className="space-y-1 md:space-y-2">
              {visibleSources?.map((source, i) => (
                <div key={i} className="text-[9px] md:text-xs text-neutral-400 truncate p-1.5 md:p-2 bg-neutral-800 rounded">
                  {source}
                </div>
              ))}
              {diagnosis.sources.length > 3 && (
                <button
                  ...
                  className="text-[10px] md:text-xs text-violet-400 hover:underline"
                >
      ...
      {/* Footer */}
      <div className="px-3 py-2.5 md:px-5 md:py-4 bg-neutral-800/50 border-t border-neutral-700">
        <div className="flex items-center justify-between">
