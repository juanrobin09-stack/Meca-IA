...
const urgencyConfig = {
  low: { label: 'Faible', color: 'bg-green-500', textColor: 'text-green-600', bgColor: 'bg-green-950/30', icon: '🟢' },
  medium: { label: 'Moyen', color: 'bg-yellow-500', textColor: 'text-yellow-600', bgColor: 'bg-yellow-950/30', icon: '🟡' },
  high: { label: 'Urgent', color: 'bg-red-500', textColor: 'text-red-600', bgColor: 'bg-red-950/30', icon: '🔴' }
}
...
export default function DiagnosticResult({ diagnosis }: DiagnosticResultProps) {
  ...
  return (
    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 overflow-hidden shadow-lg">
      {/* Header */}
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          ...
          <p className="text-neutral-200">{diagnosis.problem_identified}</p>
        </div>
        ...
        <div className="bg-neutral-800 rounded-xl p-4">
          ...
          <div className="flex items-center justify-between">
            <span className="text-neutral-400">Coût total estimé</span>
              ...
              <div className="space-y-1">
                {diagnosis.parts_needed.map((part, i) => (
                  <div key={i} className="flex justify-between text-sm">
                    <span className="text-neutral-300">{part.name}</span>
            ...
            <ol className="space-y-2">
              {diagnosis.recommendations.map((rec, i) => (
                <li key={i} className="flex items-start gap-3 text-neutral-300">
                  <span className="w-6 h-6 rounded-full bg-blue-900/30 text-blue-400 flex items-center justify-center text-xs font-medium flex-shrink-0">
                    {i + 1}
                  </span>
                  <span>{rec}</span>
                </li>
